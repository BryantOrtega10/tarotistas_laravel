...
@section('content')
    <form method="post" action="{{ route('tarotistas.editar', ['id' => $tarotista->id]) }}" enctype="multipart/form-data">
        @csrf
                    ...
                    <div class="col-md-3 col-12">
                        <div class="form-group">
                            ...
                            <select id="estado" name="estado"
                                ...
                                <option value="3" @if (old('estado', $tarotista->estado) == 3) selected @endif>Activado</option>
                                <option value="4" @if (old('estado', $tarotista->estado) == 4) selected @endif>Rechazado</option>
                                <option value="5" @if (old('estado', $tarotista->estado) == 5) selected @endif>Bloqueado</option>
                            </select>
                            @error('estado')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-3 col-12 text-center">
                        <figure class="preview-img-cont">
                            <img class="preview-img" src="{{ isset($tarotista->user->photo) ? Storage::url("public/users/".$tarotista->user->photo) : '/images/no-person.png'}}" /> 
                            <input type="file" accept="image/*" name="image" id="image" />
                        </figure>            
                        <br>
                        <button id="remove-photo-btn" class="btn btn-outline-danger">Quitar Foto</button>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3 col-12">
                                <div class="form-group">
                                    ...
                                    <input type="time" class="form-control @error('horarioInicio') is-invalid @enderror"
                                        id="horarioInicio" name="horarioInicio"
                                        value="{{ old('horarioInicio', $tarotista->txt_horario_inicio) }}">
                                    @error('horarioInicio')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="col-md-3 col-12">
                                <div class="form-group">
                                    ...
                                    <input type="time" class="form-control @error('horarioFin') is-invalid @enderror"
                                        id="horarioFin" name="horarioFin"
                                        value="{{ old('horarioFin', $tarotista->txt_horario_fin) }}">
                                    @error('horarioFin')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="col-md-3 col-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('anios_exp') is-invalid @enderror"
                                        id="anios_exp" name="anios_exp"
                                        value="{{ old('anios_exp', $tarotista->anios_exp) }}">
                                    @error('anios_exp')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                                    ...
                                    <select id="pais" name="pais" disabled
                                        ...
                                        @foreach ($paises as $pais)
                                            <option value="{{ $pais->id }}"
                                                @if (old('pais', $tarotista->fk_pais) == $pais->id) selected @endif>
                                                {{ $pais->nombre }} </option>
                                        @endforeach
                                    </select>
                                    @error('pais')
                                    ...
                                    <select id="banco" name="banco" disabled
                                        ...
                                        @foreach ($bancos as $banco)
                                            <option value="{{ $banco->id }}"
                                                @if (old('banco', $tarotista->fk_banco) == $banco->id) selected @endif>
                                                {{ $banco->nombre }} </option>
                                        @endforeach
                                    </select>
                                    @error('banco')
                            ...
                            <div class="col-md-3 col-12 @if($tarotista->banco?->ap_tipo_cuenta == 0) d-none @endif">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('tipo_cuenta') is-invalid @enderror" readonly
                                        id="tipo_cuenta" name="tipo_cuenta" @if($tarotista->tipo_cuenta == 1) value="Ahorros" @elseif($tarotista->tipo_cuenta == 2) value="Corriente" @else value="No Aplica" @endif>
                                    @error('tipo_cuenta')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="col-md-3 col-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('tipo_cuenta') is-invalid @enderror" readonly
                                        id="tipo_cuenta" name="tipo_cuenta" value="{{$tarotista->cuenta}}">
                                    @error('tipo_cuenta')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
                                        </span>
                            ...
                            <div class="col-md-12 col-12">
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control @error('descripcion_corta') is-invalid @enderror" id="descripcion_corta"
                                        name="descripcion_corta">{{ old('descripcion_corta', $tarotista->descripcion_corta) }}</textarea>
        ...
        <div class="card card-light">
            ...
            <div class="card-body">
                <button class="btn btn-outline-success d-inline-block agregar-especialidad"
                    ...
                    @foreach (old('especialidad', $tarotista->especialidades) as $index => $especialidad_item)
                        <div class="row align-items-end especialidad-item" data-id="{{ $index }}">
                            <div class="col-md-6 col-8">
                                <div class="form-group">
                                    <label for="especialidad_{{ $index }}" class="lb-especialidad">Especialidad
                                        {{ $index + 1 }}:</label>
                                    <select id="especialidad_{{ $index }}" name="especialidad[]"
                                        ...
                                        @foreach ($especialidades as $especialidad)
                                            <option value="{{ $especialidad->id }}"
                                                @if (old(
                                                        'especialidad.' . $index,
                                                        isset($tarotista->especialidades[$index]) ? $tarotista->especialidades[$index]->id : null) == $especialidad->id) selected @endif>
                                    ...
                                    @isset($tarotista->especialidades[$index])
                                        <input type="hidden" name="especialidadIDs[]"
                                            value="{{ $tarotista->especialidades[$index]?->pivot->id }}">
                                    @endisset
                                </div>
                            </div>
                            <div class="col-md-6 col-4">
                                <button type="button" class="btn btn-outline-danger quitar-especialidad mb-3"
                                    data-id="{{ $index }}">Quitar</button>
                            </div>
                        </div>
                    @endforeach
                </div>
...
@section('js')
    ...
    <script src="/js/tarotista/editar.js"></script>
@stop
